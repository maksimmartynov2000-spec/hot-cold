...
select cron.schedule('push-clean', '17 4 * * *', $$
  delete from push_outbox where sent_at < now() - interval '7 days';
  select expire_friend_requests();
$$);
```

Страница из шага 2 показывает эти строки уже с подставленным пропуском.

Второе задание чистит очередь: отправленные строки иначе копятся вечно. Неделю
держим на случай разбирательства.

Пропуск меняется тем же запросом: `cron.schedule` с тем же именем задания
переписывает старое, отдельно снимать его не нужно.
